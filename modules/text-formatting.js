...
class TextFormatting {
    ...
    applyFormat(formatType, color = null) {
        ...
        if (!activeElement || !activeElement.classList.contains('text-input')) {
            showNotification('Выделите текстовое поле для форматирования', 'warning');
            return;
        }
        ...
        if (start === end) {
            showNotification('Выделите текст для форматирования', 'warning');
            return;
        }
        ...
        showNotification(`Форматирование применено: ${formatType}${color ? ' ' + color : ''}`, 'success');
    }
...
function clearAllFormatting() {
    ...
    showNotification('Все форматирование очищено', 'success');
}
